...
public class FogDevice extends PowerDatacenter {
    ...
    /**
     * PROJECT PATCH (fog-ids / Member B):
     * iFogSim2 tracks per-device compute utilisation internally as lastUtilization
     * (fraction of host MIPS currently allocated, updated on every processing pass)
     * but never exposes it. The resource-aware IDS design in this project requires
     * a node to read its own headroom, and the security control plane (Member C)
     * needs the same figure per fog device per scheduling interval. Rather than
     * re-deriving this by reflection, we expose the existing field directly.
     * See docs/interfaces.md, "Node telemetry interface".
     */
